...
CHARACTER_MOTION = {"tunde": {"idle": (0.0, 0.0, 1.0), "talk": (0.0, -2.0, 1.0), "shock": (-6.0, -10.0, 1.04), "shocked": (-6.0, -10.0, 1.04), "freeze": (0.0, 0.0, 1.0), "dance": (4.0, -6.0, 1.03), "vibe": (-2.0, -3.0, 1.02), "check_pocket": (3.0, 4.0, 1.0), "look_at_camera": (0.0, 0.0, 1.01), "laugh": (2.0, -4.0, 1.02), "stand": (0.0, 0.0, 1.0)}, "seyi": {"idle": (0.0, 0.0, 1.0), "talk": (0.8, -1.0, 1.0), "look": (2.5, 0.0, 1.0), "turn": (5.0, 0.0, 1.0), "look_at_camera": (1.5, 0.0, 1.01), "laugh": (2.0, -2.0, 1.01), "freeze": (0.0, 0.0, 1.0)}, "mama": {"idle": (0.0, 0.0, 1.0), "stand": (0.0, 0.0, 1.0), "talk": (-1.0, 1.0, 1.01), "turn": (-4.0, 0.0, 1.0), "look_at_camera": (-1.5, 0.0, 1.02), "angry": (0.0, 2.0, 1.01), "freeze": (0.0, 0.0, 1.0)}}
...
def apply_performance(image, state, character):
    image=apply_face_performance(image,state,character); rotation,y_shift,scale=CHARACTER_MOTION.get(character,CHARACTER_MOTION["tunde"]).get(state.pose,(0.0,0.0,1.0))
    if state.pose=="dance": rotation*=math.sin(1.0)
    if scale!=1.0: image=image.resize((max(1,round(image.width*scale)),max(1,round(image.height*scale))),Image.Resampling.LANCZOS)
...
def _action_start_at(scene, character, frame_time, action):
    candidates=[float(cue.time) for cue in scene.cues() if cue.character==character and cue.time<=frame_time and cue.pose==action]
    if scene.story_plan is not None: candidates.extend(float(cue.at) for cue in cues_for(scene.story_plan,character) if cue.at<=frame_time and cue.action==action)
    return max(candidates) if candidates else None

def _conversation_focus(scene, character, frame_time):
    """Make a visible listener attend to the currently speaking cast member."""
...
def performance_for_character(scene, character, frame_time):
    ...
    resolved_expression=acting_expression(character,instance.pose,instance.expression); start=_action_start_at(scene,character,frame_time,instance.pose)
    if start is not None: phase,_,motion_progress=acting_motion(character,instance.pose,frame_time-start)
    ...
    if instance.pose=="look_at_camera": focus="camera"
    if focus is None: focus=_conversation_focus(scene,character,frame_time)
    return PerformanceState(pose=instance.pose,action=instance.pose,expression=resolved_expression,mouth=active,phase=phase,motion_progress=motion_progress,focus=focus)
