...
@lru_cache(maxsize=2)
def _background_variant(power_off: bool) -> Image.Image:
    """Build a room only once per lighting state; frames reuse the bitmap."""
    if power_off:
    ...
    if not power_off:
        ...
        image = Image.alpha_composite(image, glow.filter(ImageFilter.GaussianBlur(85)))

    vignette = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    vd = ImageDraw.Draw(vignette, "RGBA")
    vd.rectangle((0, 0, W, H), outline=(0, 0, 0, 70), width=50)
    return Image.alpha_composite(image, vignette.filter(ImageFilter.GaussianBlur(28))).convert("RGBA")
